...
<?php 
    ...
    $table =str_replace(".php"," ",$server_file);
    echo $table;
    $menu = "menu";
...
$sql ="SELECT * FROM  pageauto where table_name='$table' ORDER BY number_set";
$result = mysqli_query($conn,$sql);
$num = mysqli_num_rows($result);
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        $where_set = $where_table;
        $select_option_val =$select_option_value;
        $select_show_val =$select_show_value;
        ?>
     <?php    if("select" == settable($conn,'common_name',$type_box,'name')){?>
        <div class="col-md-6 brands">  
            <<?php echo $type_box;?> name="<?php echo $id_name; ?>" id="<?php echo $id_name; ?>" class="form-control " >
                <option val=""><?php echo $box_title; ?></option>
                <?php
                   $prods = "SELECT * FROM $table_set where $where_set AND active='1'";
   ...
   <?php  }else if($id_name =="t_name"){?>
    <div class="t_name">
         <label class="form-check-label" for="flexSwitchCheckDefault" style='width:80%;text-align:left;'><?php echo $box_title; ?></label>
          <<?php echo $type_box;?> class="<?php echo $class_name; ?>" type="<?php echo $label_type;?>" role="switch"  val="0" name='<?php echo $id_name; ?>' id='<?php echo $id_name; ?>'>
   </div>
    <?php }else{?>
    <div class="col-md-6 ">
     <label class="form-check-label" for="flexSwitchCheckDefault" style='width:99%;text-align:center;'><?php echo $box_title; ?></label><br>
     <<?php echo $type_box;?> class="<?php echo $class_name; ?>" type="<?php echo $label_type;?>" role="switch" placeholder='<?php echo $box_title; ?>'  val="0" name='<?php echo $id_name; ?>' id='<?php echo $id_name; ?>'>
   </div>
